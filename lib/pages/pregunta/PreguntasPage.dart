...
class PreguntasPage extends StatelessWidget {
  ...
  Widget _buildItem(BuildContext context, Pregunta pregunta, [int nivel = 0]) {
    ...
    children.add(Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(
          color: nivel == 0 ? Colors.black : Colors.green,
        ),
      ),
      child: Text(
        pregunta.texto,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: new Text("PREGUNTE A UN EXPERTO"),
        centerTitle: true,
      ),
      key: _scaffoldKey,
      body: FutureBuilder(
        future: this.preguntaApi.getPreguntas(),
        builder: (BuildContext futureBuildContext, AsyncSnapshot snapshot) {
            ...
            default:
              if (snapshot.hasError) {
                ...
                if (snapshot.data.length == 0) {
                  return Center(
                    child: Text(
                      "No ha realizado preguntas hasta el momento",
                    ),
                  );
                }
                return new ListView.builder(
                  itemCount: snapshot.data.length,
                  itemBuilder: (BuildContext listViewBuildContext, int index) {
                    return this
                        ._buildItem(listViewBuildContext, snapshot.data[index]);
                  },
                );
              }
          }
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => CreatePreguntaPage(persona: persona)),
          );
        },
      ),
    );
